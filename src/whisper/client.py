...
class WhisperClient:
    ...
    async def transcribe_file(
        ...
        with open(audio_path, "rb") as f:
            data.add_field(
                "audio_file", f, filename=audio_path.name, content_type="audio/wav"
            )
            params = {"task": task, "output": output_format, "encode": "true"}
            if language:
            ...
            try:
                async with self.session.post(
                    f"{self.base_url}/asr", data=data, params=params
                ) as response:
                    if response.status == 200:
    ...
    async def transcribe_bytes(
        ...
        data = aiohttp.FormData()
        data.add_field(
            "audio_file", audio_data, filename=filename, content_type="audio/wav"
        )
        params = {"task": task, "output": output_format, "encode": "true"}
        if language:
            params["language"] = language
        ...
        try:
            logger.debug(
                f"Sending {len(audio_data)} bytes to Whisper at {self.base_url}/asr"
            )
            async with self.session.post(
                f"{self.base_url}/asr", data=data, params=params
            ) as response:
            ...
            try:
                async with self.session.post(
                    ...
                    if response.status == 200:
                        result = await response.json()
                        logger.info(
                            f"Language detection successful for {audio_path.name}"
                        )
                        return result
                    else:
                        error_text = await response.text()
                        logger.error(
                            f"Language detection failed: {response.status} - {error_text}"
                        )
                        raise Exception(
                            f"Language detection failed: {response.status} - {error_text}"
                        )
...
async def transcribe_audio_file(
    file_path: Union[str, Path], whisper_url: str = "http://localhost:9000", **kwargs
) -> Dict[str, Any]:
    """
