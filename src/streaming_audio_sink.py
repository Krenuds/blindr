...
class TimeoutManager:
    ...
    def schedule_timeout(self, user_id: int, timeout_duration: float, handler_coroutine):
...
class StreamingAudioSink(discord.sinks.Sink):
    ...
    async def process_audio_segment(self, user_id: int, audio_segment: bytes):
        ...
        try:
            ...
            wav_data = self.audio_processor.pcm_to_wav(audio_segment, self.config['sample_rate'])
    ...
    async def timeout_handler(self, user_id: int, timeout_duration: float):
        ...
        try:
            ...
            current_time = time.time()
            time_since_last_packet = current_time - self.buffer_manager.user_last_packet_time.get(user_id, 0)
